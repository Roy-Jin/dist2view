    ...
    if (node.isDirectory) {
            ...
            >
              ...
              <div className="opacity-0 group-hover:opacity-100 flex items-center gap-1 transition-opacity shrink-0" onClick={(e) => e.stopPropagation()}>
                ...
                <button
                  title="Delete folder"
                  onClick={() => {
                    if (confirm(`确定要删除文件夹 ${node.name} 及其下的所有文件吗？`)) {
      ...
      return (
        <div 
          ...
          className={`group flex items-center justify-between px-2.5 py-1.5 rounded-xl text-xs cursor-pointer transition-all select-none ${
            isSelected 
      ...
      {isCreatingInFolder === '' && (
        <div className="p-3 bg-white/1 border-b border-white/5 animate-fade-in">
          <form onSubmit={(e) => handleCreateSubmit(e, '')} className="flex items-center gap-2">
            ...
            <input 
              type="text" 
              placeholder={creatingType === 'folder' ? "Folder name..." : t('fileNamePlaceholder')} 
              autoFocus
              value={newFileName}
              onChange={(e) => setNewFileName(e.target.value)}
              onKeyDown={handleKeyDown}
        ...
        >
          {contextMenu.isDirectory ? (
            <>
              ...
              <button 
                onClick={() => {
                  if (confirm(`确定要删除整个文件夹 ${contextMenu.path} 及其所有文件吗？`)) {
            ...
            <>
              ...
              <button 
                onClick={() => {
                  if (confirm(`确定要删除文件 ${contextMenu.path} 吗？`)) {
